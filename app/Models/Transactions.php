...
{
    protected $guarded = [];
    protected $appends = ['company_code','location_from','location_from','warehouse_name','po_warehouse_id','warehouse_po_name', 'location_name', 'user_name','supplier_name', 'warehouse_receive_name','to_warehouse','pmcsout_warehouse','journal_name','journal_description','ppn_item','so_code','customer_id','customer_name','ref','transaction_code', 'pmcs_out_code','po_number','pr_code', 'code_ticket'];
    ...
    public function location() {
      ...
      public function overheads(){
          return $this->hasMany(Overheads::class, 'transaction_id', 'id');
      }
    ...
    public function expired_dates(){
          return $this->hasMany(Expired_dates::class, 'transaction_id', 'id');
      }
  
     public function trans(){
          return $this->hasMany(Transactions::class, 'iptn_id', 'id');
      }
      
      public function trans_rework(){
          return $this->hasMany(Transactions::class, 'iptn_out_id', 'id');
      }
      
      public function rental_ticketing(){
          return $this->hasMany(Rental_ticketings::class, 'id', 'rental_ticketing_id');
      }
      ...
      public function inventorys(){
          return $this->hasMany(Inventorys::class, 'transaction_id', 'id')
          ->select('inventory.*','unit_convert.factor', 'unit_convert.multiplier')
                  ->leftJoin('unit_convert', function ($join) {
                                      $join->on('inventory.item_code','=','unit_convert.item_code')
                                            ->on('inventory.item_unit','=','unit_convert.from_unit' );
                                  });
      }
  
    public function inventory_scraps(){
          return $this->hasMany(Inventorys::class, 'transaction_id', 'id')
      ->select('inventory.*','unit_convert.factor', 'unit_convert.multiplier','stock.qty as stock','stock.avg_price as avg_price','transaction.location_id')
          ...
          ->leftJoin('unit_convert', function ($join) {
           ...
           ->leftJoin('stock', function ($join){
            $join->on('inventory.item_code','=','stock.item_code')
                 ->on( 'transaction.warehouse_id','=','stock.warehouse_id');
              });
      }
      ...
      public function transaction_pmcs_out() {
     ...
     public function getPmcsOutCodeAttribute() {
        $transaction_pmcs_out = $this->transaction_pmcs_out()->first();
        return ($transaction_pmcs_out?$transaction_pmcs_out->code:null);
     }
     ...
     public function getPoNumberAttribute() {
        $po = $this->purchase_order_number()->first();
        return ($po?$po->number_po:null);
     }
     ...
     public function getCodeTicketAttribute() {
        $rental_ticketing = $this->rental_ticketing()->first();
        return ($rental_ticketing?$rental_ticketing->code:null);
     }
   ...
   public static function getNextCounterPLId() {
     ...
     public static function getNextCodeITOut() {
       ...
       if($transaction) {
         $data = explode($CODE.$IPO.$TAHUN_BULAN_TANGGAL.$IPO, $transaction->code);
         $last_count = intval($data[1]) + 1;
       }
     ...
     public static function getNextCounterReturBarcodeId() {
