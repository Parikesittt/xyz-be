...
class Price_lists extends Model
{
    protected $guarded = [];
    protected $appends = ['location_name','supplier_name','validate_date_from_name','validate_date_to_name','location_code','supplier_name','user_name'];
	...
	public function price_list_locations(){
        return $this->hasMany(Price_list_locations::class, 'price_list_id', 'id');
    }
	...
	public static function getNextCounterId() {
      ...
      // get last count
      $price_list = Price_lists::select('code')
      ...
      if($price_list) {
        $data = explode($MC.$TAHUN_BULAN_TANGGAL, $price_list->code);
