...
{
    protected $guarded = [];
    protected $appends = ['po_internal_id','po_warehouse_id','warehouse_po_name','part_no','item_no','so_code','so_id','customers_id','customer_name','location_code','warehouse_receive_name','warehouse_name','location_name','warehouse_code','department_name','supplier_id','supplier_name','pr_code','transaction_type_name','warehouse_send_id','warehouse_send_name','qty_po','unit_po','qty_so','unit_so','sales_tax_group','purchase_order_id','po_code', 'class_item', 'item_masa'];

    public function class_items() {
		return $this->belongsTo(Items::class, 'item_code', 'id');
	}
	...
	public function getClassItemAttribute() {
       $items = $this->class_items()->first();
       return ($items?$items->item_class:null);
    }
	
	public function getItemMasaAttribute() {
       $items = $this->class_items()->first();
       return ($items?$items->masa_item:null);
    }
}
